...
export class ApiService {
  ...
  getBlog() {
    return this.http.get(this.base_path).pipe(retry(2), catchError(this.handleError));
  }
  getSingle(blog: any[]) {
    // this.list = blog;
    console.log(blog);
    return this.http.get(`https://epower.ng/wp-json/wp/v2/posts/${blog}`).pipe(retry(2), catchError(this.handleError));
  }
  getUserData(blogs) {
    console.log(blogs);
